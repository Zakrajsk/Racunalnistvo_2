...
def oklepaji_dinamicno(izraz):
    ...
    for i in range(n): 
        vrednosti[i][i] = (izraz[2*i], izraz[2*i])
    ...
    for i in range(1, n):  # katera naddiagonala
        for k in range(n-i):  # kateri zaporedni elt v naddiagonali
            ...
            najmanjsa = float('inf')
            for j in range(i):
                levo_max, levo_min = vrednosti[i][k]
                desno_max, desno_min = vrednosti[i][]
                op = izraz[###]
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyNzM3MSwidXNlciI6MjA5OX0:1lHAlS:ojlQctV-yesoN2XlglXMt6ykDGo'
        try:
            tests = [
                    ("oklepaji('')", (0, 0)),
                    ("oklepaji('-5')", (-5, -5)),
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyNzM3MiwidXNlciI6MjA5OX0:1lHAlS:8IDZ9qsZQ41u3RW24lsqtRF9pJY'
        try:
            tests = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyNzM3MywidXNlciI6MjA5OX0:1lHAlS:OUenjTkHSjHXj9IJwO8Yv69Esks'
        try:
            tests = [
                    ("oklepaji_izraz('')", ''),
                    ("oklepaji_izraz('-5')", '-5'),
