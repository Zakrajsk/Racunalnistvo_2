...
def zabica(mocvara):
    ...
    def z(e, i):
        """
        Funkcija izracuna minimalno stevilo skokov potrebnih da zabica preskoci mocvaro. 
        """
        if i >= len(mocvara):
            return 0
        ...
        for k in range(1, e + 1):
            if k + i < len(mocvara):
                najmanjsa = min(najmanjsa, z(e - k + mocvara[i + k], i + k))
            else:
                return 1

        return najmanjsa + 1

    return z(mocvara[0], 0)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjozMDA1NSwidXNlciI6MjA5OX0:1nKc3e:cv_UlGNbfWsO0r-9jlZGvtQiPmI'
        try:
